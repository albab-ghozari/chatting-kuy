		...
		if (!isFromMe && (!isActiveConv || !document.hasFocus())) {
			...
			notify({
				senderName: conv?.isGroup
					? `${msg.sender?.username} @ ${conv.name}`
					: (msg.sender?.username ?? 'Pesan baru'),
		...
		conversations = conversations.map((c) =>
			Number(c.id) === Number(updated.id)
				? {
						...
						groupAvatar: updated.groupAvatar !== undefined ? updated.groupAvatar : c.groupAvatar,
...
>
	<!-- SIDEBAR -->
	<aside
		class="flex w-full flex-col border-r border-gray-100 bg-white md:w-72 md:shrink-0
			...
			<div class="mx-3 mb-3 rounded-xl border border-gray-200 bg-gray-50 p-3">
				<div class="mb-3 flex rounded-lg bg-gray-200 p-0.5">
					<button
						...
						class="flex-1 rounded-md py-1 text-xs font-semibold transition-colors {newChatMode ===
						'dm'
							? 'bg-white text-[#0d0f1e] shadow-sm'
							: 'text-gray-500'}">Chat</button
					>
					<button
						on:click={() => {
							newChatMode = 'group';
					...
					{#if selectedGroupMembers.length > 0}
						<div class="mb-2 flex flex-wrap gap-1">
							{#each selectedGroupMembers as m}
								<span
									class="flex items-center gap-1 rounded-full bg-[#0d0f1e] px-2 py-0.5 text-xs text-white"
									>{m.username}<button
										on:click={() => toggleGroupMember(m)}
										class="ml-0.5 opacity-70 hover:opacity-100">×</button
									></span
								>
							{/each}
						</div>
					...
					<div class="mt-2 flex flex-col gap-0.5">
						{#each userResults as u (u.id)}
							...
							<button
								...
								class="flex items-center gap-2 rounded-lg px-2 py-2 text-left hover:bg-white disabled:opacity-50 {selected
									? 'bg-indigo-50'
									: ''}"
							>
				...
				{#if newChatMode === 'group' && selectedGroupMembers.length > 0 && groupNameInput.trim()}
					<button
						on:click={startGroup}
						disabled={startingChat}
						class="mt-3 w-full rounded-xl bg-[#0d0f1e] py-2 text-sm font-semibold text-white hover:opacity-80 disabled:opacity-50"
					>
			...
			{:else if filtered.length === 0}
				<div class="flex flex-col items-center justify-center gap-2 py-16">
					...
					{#if !searchQuery}<button
							...
							class="text-xs font-semibold text-[#0d0f1e] underline underline-offset-2"
	...
	<!-- Area chat -->
	<main
		class="flex w-full min-w-0 flex-1 flex-col overflow-hidden md:w-auto
	...
	>
		{#if activeConversation}
			...
			>
				<button
					aria-label="backSideBar"
					on:click={backToSidebar}
					class="-ml-1 flex h-8 w-8 items-center justify-center rounded-xl text-gray-500 hover:bg-gray-100"
				>
					<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
						><path
							stroke-linecap="round"
					...
					{#if activeConversation.isGroup}
						<button on:click={() => (mobileView = 'groupinfo')} class="w-full text-left">
							...
							<p class="text-xs text-gray-400">
								{(activeConversation.members ?? []).length} anggota · ketuk untuk info
							</p>
						</button>
					{:else}
						<p class="truncate text-sm font-semibold text-[#0d0f1e]">{activeConversation.name}</p>
			...
			<ChatWindow
				...
				isOnline={!activeConversation.isGroup &&
					onlineUserIds.has(Number(activeConversation?.otherUserId))}
			...
			<GroupInfoPanel
				...
				on:memberRemoved={(e) => {
					...
					if (activeConversation?.members) {
						activeConversation = {
							...activeConversation,
							members: activeConversation.members.filter((m) => m.id !== userId)
						};
					}
					conversations = conversations.map((c) =>
